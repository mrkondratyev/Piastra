...
Spatial reconstruction is performed on the 4-velocity components  uⁱ = W vⁱ
...
method ('flux-CT') so that ∇·B = 0 is preserved to machine precision.
...
Notes
-----
  The primitive-variable recovery after each RK stage (cons2prim_sr_MHD)
  is currently a stub; see rMHD_phys.py for the inversion recipe.  Until
  that stub is filled the solver will raise NotImplementedError at run-time.
...
def _prim_recovery(state, Ngc, eos):
    """Call cons2prim_sr_MHD and write results back into state.{dens,vel*,pres,bfi*}."""
    (state.dens[Ngc:-Ngc, Ngc:-Ngc],
     ...
     state.bfi3[Ngc:-Ngc, Ngc:-Ngc]) = \
        cons2prim_sr_MHD(
            ...
            state.pres[Ngc:-Ngc, Ngc:-Ngc], eos)
...
def flux_calc_rMHD_CT(grid, MHD, par, eos):
    ...
    Reconstruction is performed on the 4-velocity  uⁱ = W vⁱ  (same
    ...
    # ----------------------------------------------------------------
    # CT: electric field at cell corners  E_3 = -(v × B)_3
    # (The electric field is obtained by averaging the face fluxes of the
    #  transverse B components, exactly as in MHD_one_step_CT.py.)
    # ----------------------------------------------------------------
...
def boundCond_electric_field_rMHD(grid, Efld3x1, Efld3x2, BC):
    ...
    ----------
    grid    : Grid
    Efld3x1 : ndarray  E_z on x1-faces  (used for Faraday's law along x2)
    Efld3x2 : ndarray  E_z on x2-faces  (used for Faraday's law along x1)
    BC      : list of 4 str
