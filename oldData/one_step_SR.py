...
@author: mrkondratyev
"""
from ideal_hydro_functions_SR import prim2cons_idealHydro_SR, cons2prim_idealHydro_SR
from boundaries import boundCond 
from reconstruction import VarReconstruct 
import numpy as np 
import copy 
from flux_functions import FluxCalcRS_SR
import time
...
in the function "oneStep_hydro_NR" we call all the key ingredients of our compressible non-relativistic fluid simulations.
...
def oneStep_hydro_SR(grid, fluid, eos, dt, rec_type, flux_type, RK_order):
    ...
    if (RK_order == 'RK2'):
        ...
        fluid_h.dens[Ngc:-Ngc, Ngc:-Ngc], fluid_h.vel1[Ngc:-Ngc, Ngc:-Ngc], \
            ...
            fluid_h.pres[Ngc:-Ngc, Ngc:-Ngc] = cons2prim_idealHydro_SR(fluid_h.mass, 
            fluid_h.mom1, fluid_h.mom2, fluid_h.mom3, fluid_h.etot, fluid.pres[Ngc:-Ngc, Ngc:-Ngc], eos) 
        
        #2nd Runge-Kutta iteration - corrector stage
        ResM, Res1, Res2, Res3, ResE = flux_calc_hydro_SR(grid, fluid_h, rec_type, flux_type, eos)
    ...
    fluid.dens[Ngc:-Ngc, Ngc:-Ngc], fluid.vel1[Ngc:-Ngc, Ngc:-Ngc], \
        ...
        fluid.pres[Ngc:-Ngc, Ngc:-Ngc] = cons2prim_idealHydro_SR(fluid.mass, 
        fluid.mom1, fluid.mom2, fluid.mom3, fluid.etot, fluid_h.pres[Ngc:-Ngc, Ngc:-Ngc], eos)
...
def flux_calc_hydro_SR(grid, fluid, rec_type, flux_type, eos):
    
    #fill the ghost cells
    fluid = boundCond(grid, fluid)
    ...
    #here we calculate the 4-velocity components 
    gamma = np.sqrt(1.0 - fluid.vel1 ** 2 - fluid.vel2 ** 2 - fluid.vel3 ** 2)
    relv1 = fluid.vel1 / gamma 
    relv2 = fluid.vel2 / gamma
    relv3 = fluid.vel3 / gamma
    ...
    if (grid.Nx1 > 1): #check if we even need to consider this dimension
        ...
        vel1_rec_L, vel1_rec_R = VarReconstruct(relv1, grid, rec_type, 1)
        vel2_rec_L, vel2_rec_R = VarReconstruct(relv2, grid, rec_type, 1)
        vel3_rec_L, vel3_rec_R = VarReconstruct(relv3, grid, rec_type, 1)
        
        #here we return to usual 3-velocities
        gamma_L = np.sqrt(1.0 + vel1_rec_L ** 2 + vel2_rec_L ** 2 + vel3_rec_L ** 2)
        gamma_R = np.sqrt(1.0 + vel1_rec_R ** 2 + vel2_rec_R ** 2 + vel3_rec_R ** 2)
        
        vel1_rec_L = vel1_rec_L / gamma_L
        vel2_rec_L = vel2_rec_L / gamma_L
        vel3_rec_L = vel3_rec_L / gamma_L
        
        vel1_rec_R = vel1_rec_R / gamma_R
        vel2_rec_R = vel2_rec_R / gamma_R
        vel3_rec_R = vel3_rec_R / gamma_R
        
        #fluxes calculation with approximate Riemann solver (see flux_type) in 1-dim
        Fmass, Fmomx, Fmomy, Fmomz, Fetot = \
            FluxCalcRS_SR(dens_rec_L, dens_rec_R, vel1_rec_L, vel1_rec_R, \
    ...
    if (grid.Nx2 > 1): #check if we even need to consider this dimension
        ...
        vel1_rec_L, vel1_rec_R = VarReconstruct(relv1, grid, rec_type, 2)
        vel2_rec_L, vel2_rec_R = VarReconstruct(relv2, grid, rec_type, 2)
        vel3_rec_L, vel3_rec_R = VarReconstruct(relv3, grid, rec_type, 2)
